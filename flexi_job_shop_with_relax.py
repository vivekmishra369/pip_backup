...
def flexible_jobshop_gurobi():
    ...
    # Set parameters to prioritize feasibility over optimality
    model.setParam(GRB.Param.MIPFocus, 1)  # Focus on finding feasible solutions
    model.setParam(GRB.Param.TimeLimit, 600)  # Set a time limit of 10 minutes

    # Solve the model
    model.optimize()

    # If the solution is infeasible, apply FeasRelax to find a feasible one
    if model.status == GRB.INFEASIBLE:
        print("Model is infeasible. Applying FeasRelax to find a feasible solution.")
        model.feasRelaxS(0, True, False, True)
        model.optimize()

    # Output the results
    if model.status == GRB.OPTIMAL or model.status == GRB.FEASIBLE:
        print(f"\nBest feasible makespan: {makespan.x}\n")
        for j in range(num_jobs):
            print(f"Job {j}:")
            for t, task in enumerate(jobs[j]):
                start_time = s[j, t].x
